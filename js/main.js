...
const validateForm = () => {
  ...
  //   errorMessages[4].innerText = "Please confirm you're not a robot";
  //   recaptchaContainer.classList.add("error-border");
  //   errorFlag = true;
  // }
  if (!errorFlag) {
    ...
    setTimeout(() => {
      sendMail(captchaValue);
    }, 3000);
  }
};

const onloadCallback = (token) => {
  captchaValue = token;
}
...
const sendMail = () => {
  let params = {
    ...
    honey: document.getElementsByName("honey"),
    "g-recaptcha-response": captchaValue,
  };
  const serviceID = "service_7ya0h75";
  const templateID = "template_4g0xpkd";
  emailjs
    ...
    .then((res) => {
      ...
      formSubject.value = "";
      formMessage.value = "";
      document.getElementsByName("honey").value = "";
      console.log(res);
    })
